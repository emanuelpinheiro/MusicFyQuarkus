...
public AlbumResponseDTO update(AlbumDTO dto, Long id) {
    ...
    @Override
    public List<AlbumResponseDTO> getAll(int page, int pageSize) {
        List<Album> list = repository
                                .findAll()
                                .page(page, pageSize)
                                .list();
        
        return list.stream().map(e -> AlbumResponseDTO.valueOf(e)).collect(Collectors.toList());
    }
